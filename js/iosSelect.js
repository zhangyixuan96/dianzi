/**
 * IosSelect
 * @param {number} level 選擇的層級 1 2 3 4 5 6 最多支持6層 
 * @param {...Array} data [oneLevelData[, twoLevelData[, threeLevelData[, fourLevelData[, fiveLevelData[, sixLevelData]]]]]] 可以用數組，也可以用方法
 * @param {Object} options
 * @param {string=} options.container 組件插入到該元素下 可選
 * @param {Function} options.callback 選擇完畢後的回調函數
 * @param {Function} options.fallback 選擇取消後的回調函數
 * @param {Function} options.maskCallback 點擊背景層關閉組件時觸發的方法
 * @param {string=} options.title 選擇框title
 * @param {number=} options.itemHeight 每壹項的高度，默認 35
 * @param {number=} options.itemShowCount 組件展示的項數，默認 7，可選3,5,7,9，不過不是3,5,7,9則展示7項
 * @param {number=} options.headerHeight 組件標題欄高度 默認 44
 * @param {css=} options.cssUnit px或者rem 默認是px
 * @param {string=} options.addClassName 組件額外類名 用於自定義樣式
 * @param {...Array=} options.relation 數組 [oneTwoRelation, twoThreeRelation, threeFourRelation, fourFiveRelation] 默認值：[0, 0, 0, 0, 0, 0]
 * @param {number=} options.relation.oneTwoRelation 第壹列和第二列是否通過parentId關聯
 * @param {number=} options.relation.twoThreeRelation 第二列和第三列是否通過parentId關聯
 * @param {number=} options.relation.threeFourRelation 第三列和第四列是否通過parentId關聯
 * @param {number=} options.relation.fourFiveRelation 第四列和第五列是否通過parentId關聯
 * @param {number=} options.relation.fiveSixRelation 第五列和第六列是否通過parentId關聯
 * @param {string=} options.oneLevelId 第壹級選中id
 * @param {string=} options.twoLevelId 第二級選中id
 * @param {string=} options.threeLevelId 第三級選中id
 * @param {string=} options.fourLevelId 第四級選中id
 * @param {string=} options.fiveLevelId 第五級選中id
 * @param {string=} options.sixLevelId 第六級選中id
 * @param {boolean=} options.showLoading 如果妳的數據是異步加載的，可以使用該參數設置為true，下拉菜單會有加載中的效果
 * @param {boolean=} options.showAnimate 是否需要入場動畫和退場動畫，如需自定義動畫效果，請修改css
 */
(function() {
	...
	IosSelect.prototype = {
		...
		initLayer: function () {
			var self = this;
			var sureText = this.options.sureText || '確定';
